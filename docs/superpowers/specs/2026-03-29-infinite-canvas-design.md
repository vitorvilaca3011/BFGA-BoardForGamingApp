...
- Pointer handling for pan: middle-click or hand-tool drag translates `Pan`.
- Pointer handling for zoom: mouse wheel adjusts `Zoom`, zooming toward cursor position.
...
**Changes:**
- Remove `viewport.ZoomBorder.ZoomChanged` subscription. Subscribe to a new `BoardViewport.ZoomChanged` event instead.
...
| Gesture | Action |
|---------|--------|
| Mouse wheel | Zoom toward cursor (0.2x - 3.0x range) |
| Middle-click drag | Pan |
| Hand tool + left-click drag | Pan |
| Zoom slider (BottomBar) | Zoom toward viewport center |
| Zoom +/- buttons | Zoom toward viewport center by 10% steps |

ZoomBorder currently handles mouse wheel zoom and right-click pan. The new implementation replaces this with equivalent behavior on BoardViewport, handling pointer events directly.
